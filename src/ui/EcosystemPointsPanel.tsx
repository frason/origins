  ...
  return (
    <section aria-label="Open-ended ecosystem points" style={{ border: '1px solid #4b4540', padding: '0.6rem', marginTop: '0.7rem', background: '#252728' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: '0.5rem' }}>
        <strong style={{ color: 'var(--sim-color-screen-highlight)', fontSize: '0.78rem' }}>Ecosystem points</strong>
        <output style={{ color: '#f0d39d', fontSize: '1.1rem', fontWeight: 700 }}>{points.total.toLocaleString()}</output>
      </div>
      <p style={{ color: 'var(--sim-color-screen-ink-muted)', fontSize: '0.64rem', margin: '0.2rem 0 0.4rem' }}>Open-ended evidence of sustained, varied, evolving life—not a perfect score.</p>
      {Object.entries(points.breakdown).map(([category, value]) => (
        <div key={category} style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--sim-color-screen-ink-soft)', fontSize: '0.67rem', padding: '0.1rem 0' }}>
