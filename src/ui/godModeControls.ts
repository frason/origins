...
export const GOD_MODE_GROUPS: GodModeControlGroup[] = [
  ...
  },
  {
    ...
    controls: [
      ...
      { label: 'Predation Hunger Threshold', key: 'predationHungerThresholdShare', min: 0.1, max: 1, step: 0.05, formatter: fixed(2), description: 'Energy-capacity share below which predators will make a kill. Lower values reduce surplus hunting but may starve predators; higher values increase prey turnover and carrion.' },
  ...
  },
  {
    ...
    controls: [
      ...
      { label: 'Minimum Carrion Energy', key: 'minimumCarrionEnergy', min: 0, max: 100, step: 2, description: 'Edible structural energy retained when an animal dies with empty reserves. Higher values sustain scavengers after starvation, but return more matter to the food web and can soften scarcity.' },
  ...
  },
  {
    ...
    controls: [
      ...
      { label: 'Monoculture Mortality', key: 'monocultureMortalityPenalty', min: 0, max: 0.5, step: 0.01, formatter: fixed(2), description: 'Optional legacy death pressure for species above the dominance threshold. It is off by default because local scarcity and reproduction restraint are more ecological; enabling it can cause abrupt declines.' },
      ...
      { label: 'Population Safety Limit', key: 'maxGlobalPopulation', min: 50, max: 2000, step: 50, description: 'Deterministic performance guard, not ecological carrying capacity. At the limit, births wait for deaths while local food, habitat, and crowding continue selecting survivors. Higher limits permit larger worlds at greater processing cost.' },
      { label: 'Overcrowding Mortality', key: 'overcrowdingMortalityRate', min: 0, max: 0.5, step: 0.01, formatter: fixed(2), description: 'Emergency mortality pressure when an imported or externally changed population exceeds the safety limit. Higher rates restore performance faster but can create sudden population crashes.' },
    ],
  },
];
