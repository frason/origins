...
and fix it by hand. #280 and #281 fix the two specific root causes found so far (label-write
...
- The watchdog lists all open issues currently labeled agent-doing or agent-review (via
  ...
  - If git log shows a commit already referencing that issue number as closed/landed (same
    check style as #281), relabel to agent-done and close the issue (reusing the same commit/
